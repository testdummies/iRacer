...
def execute_manoeuvre(time_list, gear_list, direction_list):
    ...
    command = math.get_combined_value("STOP",  0)
    bt.send_command(command, 0.01)
...
def three_point_turn_left():
    sq_time = [2, 2, 1, 2, 1, 0.8, 0.5]#0.4
    sq_gear = [1, 1, 1, 1, 1, 1,1]
    sq_direction = ["STOP", "FL", "STOP", "RR", "STOP", "FL", "FS"]
    execute_manoeuvre(sq_time, sq_gear, sq_direction)
...
def parallel_parking_right_side():
    ...
    sq_time = [2, 1.3, 1, 0.8, 1, 0.8, 1, 0.3]  # sequence time etc.
...
def parallel_parking_left_side():
    sq_time = [2, 1.3, 1, 0.8, 1, 0.6, 1, 0.4]
    sq_gear = [1, 1, 1, 1, 1, 1, 1, 1]
